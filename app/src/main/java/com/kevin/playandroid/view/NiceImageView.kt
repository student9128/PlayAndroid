...
import com.kevin.playandroid.util.DisplayUtils.dip2px
import com.kevin.playandroid.util.DisplayUtils.dp2px
...
class NiceImageView @JvmOverloads constructor(
    ...
    private var isCircle: Boolean = false // 是否显示为圆形，如果为圆形则设置的corner无效 = false
    private var isCoverSrc: Boolean = false // border、inner_border是否覆盖图片 = false
    private var borderWidth: Int = 0 // 边框宽度 = 0
    private var borderColor = Color.WHITE // 边框颜色
    private var innerBorderWidth: Int = 0 // 内层边框宽度 = 0
    private var innerBorderColor = Color.WHITE // 内层边框充色
    private var cornerRadius: Int = 0 // 统一设置圆角半径，优先级高于单独设置每个角的半径 = 0
    private var cornerTopLeftRadius: Int = 0 // 左上角圆角半径 = 0
    private var cornerTopRightRadius: Int = 0 // 右上角圆角半径 = 0
    private var cornerBottomLeftRadius: Int = 0 // 左下角圆角半径 = 0
    private var cornerBottomRightRadius: Int = 0 // 右下角圆角半径 = 0
    private var maskColor: Int = 0 // 遮罩颜色 = 0
    ...
    fun setCornerBottomRightRadius(cornerBottomRightRadius: Int) {
        this.cornerBottomRightRadius =
            dp2px(context, cornerBottomRightRadius.toFloat())
        calculateRadiiAndRectF(true)
    }
